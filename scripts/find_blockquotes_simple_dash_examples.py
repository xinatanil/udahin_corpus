...
IMMEDIATE_XR_RE = re.compile(
    rf'^[{WORD_CHARS}\'-]+\s+[{WORD_CHARS}\'-]+-\s+(?:см\.\s*<wordLink\b|то же, что\s*<wordLink\b)'
)
SOURCE_TARGET_RE = re.compile(
    rf'^(?P<source>[{WORD_CHARS}\'-]+\s+[{WORD_CHARS}\'-]+-)\s+(?P<target>(?:см\.\s*<wordLink\b|то же, что\s*<wordLink\b).*)$',
    re.S,
)
...
def matches_rule(blockquote_xml: str) -> bool:
    ...
    if visible_text.startswith('('):
        return False
    return second_word_ends_with_dash(visible_text) and IMMEDIATE_XR_RE.match(blockquote_xml.strip()) is not None
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Dump simple blockquotes where the first word ends with a dash and the rest looks like Russian text.'
    )
    parser.add_argument('--input', default='./chatGPT_exp/converted_dict.xml', help='Input XML file to scan')
    parser.add_argument(
        '--output',
        default='./chatGPT_exp/blockquotes_simple_dash_examples.txt',
    ...
    if args.apply_output:
        ...
        print('Rule: second visible word ends with a dash, immediately followed by "см. <wordLink>" or "то же, что <wordLink>"')
