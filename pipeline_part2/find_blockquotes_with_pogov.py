...
def build_excluded_labels(ignore_case: bool) -> set[str]:
...
def find_split(raw_content: str, ignore_case: bool) -> tuple[str, str] | None:
    ...
    excluded_labels = build_excluded_labels(ignore_case)
...
def collect_matches(xml: str, ignore_case: bool) -> list[tuple[str, str]]:
    matches: list[tuple[str, str]] = []
    for match in BLOCKQUOTE_RE.finditer(xml):
        split = find_split(match.group('content'), ignore_case)
...
def apply_splits(xml: str, ignore_case: bool) -> tuple[str, int]:
    applied = 0
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Find blockquote splits using the divider word "погов.".')
    parser.add_argument('--input', default='./pipeline_output/converted_dict.xml', help='Input XML file to scan')
    parser.add_argument('--output', default='./pipeline_output/blockquotes_with_pogov.txt', help='Output text file')
    ...
    if args.apply_output:
        ...
        new_xml, applied = apply_splits(xml, args.ignore_case)
        out_path.write_text(new_xml, encoding='utf-8')
        print(f'Input: {input_path}')
        print(f'Word: {DIVIDER}')
        print(f'Applied: {applied}')
        print(f'Output: {out_path}')
        return 0

    matches = collect_matches(xml, args.ignore_case)
