...
RULES = [
    {
        ...
        'source_target_re': re.compile(
            ...
            rf'(?P<target>(?:см\.\s*<wordLink\b|то же, что\s*<wordLink\b).*)$',
            re.S,
        ),
...
def match_rule(blockquote_xml: str) -> tuple[dict, tuple[str, str]] | None:
    ...
    for rule in RULES:
        ...
        if match is None:
            continue
        return rule, (match.group('source').strip(), match.group('target').strip())
    return None
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Dump or apply simple xref blockquotes that can be converted into <ex> entries.'
    )
    parser.add_argument('--input', default='./pipeline_output/converted_dict.xml', help='Input XML file to scan')
    parser.add_argument(
        '--output',
        default='./pipeline_output/blockquotes_simple_xr_examples.txt',
    ...
    if args.apply_output:
        ...
        print('Rule set: simple xref examples')
        print_counts(counts)
